...
export default function StatusPage({ params }: { params: { id: number } }) {
                        ...
                        <TooltipProvider delayDuration={200} key={index}>
                            <Tooltip>
                                ...
                                <TooltipContent side='bottom' className='bg-popover drop-shadow-lg border-0 duration-0'>
                                    <p className='font-semibold text-gray-500'>{stat.day}</p>
                                    <p className='text-gray-500'>Uptime {stat.uptime}%</p>
                                </TooltipContent>
                            </Tooltip>
                        </TooltipProvider>
                    )).reverse()}
    ...
    if (!pageData) {
        return <div className="flex justify-center items-center h-screen text-primary">No data available</div>;
    }

    return (
        <>
            <main className="container flex flex-col gap-2 lg:w-[58vw] items-center mx-auto py-8">
                <h1 className="text-3xl font-bold mb-6 text-primary">{pageData.name}</h1>

                {openIncidents.length > 0 ? (
                    <div className="bg-gray-800 p-4 w-full rounded-lg shadow-2xl">
                        <h2 className="text-3xl font-semibold text-primary">Ongoing Incidents</h2>
                    ...
                    <div className="bg-card p-4 mb-6 w-full rounded-lg shadow-2xl">
                        <div className='flex items-center gap-4'>
                            ...
                            <h2 className="text-3xl font-semibold text-primary"> All systems <span className='text-green-500'>operational</span></h2>
                        </div>
                    </div>
                )}
                        ...
                        <div key={component.id} className="bg-card rounded-lg p-4 shadow-2xl">
                            <div className='flex justify-between items-center w-full'>
                                <div className='flex gap-2 items-center'>
                                    <h2 className="text-xl font-semibold text-primary">{component.name}</h2>
                                ...
                                <p className={`${component.status == 1 ? "text-green-400" : component.status == 2 ? "text-yellow-400" : component.status == 3 ? "text-orange-400" : component.status == 4 ? "text-red-400" : component.status == 5 ? "text-blue-400" : ""} font-semibold dark:font-light text-sm`}>
                ...
                <div className="bg-card p-4 w-full rounded-lg shadow-2xl">
                    <h2 className="text-2xl font-semibold text-primary">Incident History (Last 14 Days)</h2>
                    {Object.entries(incidentsByDay).map(([day, incidents]) => (
                        <div key={day} className="mt-4">
                            <h3 className="text-lg font-bold text-primary">{dayjs(day).format('MMMM D, YYYY')}</h3>
                            <div className='w-full h-[1px] my-2 bg-gray-500' />
                            {incidents.length > 0 ? (
                                <div className="flex flex-col gap-2 py-2 rounded-lg mt-2">
                                    {incidents.map((incident) => (
                                        <div key={incident.id} className='bg-neutral-500 rounded-lg p-4'>
                                            <p className={`font-semibold text-xl ${incident.severity == "Minor" ? "text-yellow-600" : incident.severity == "Major" ? "text-orange-600" : incident.severity == "Critical" ? "text-red-600" : ""}`}>
                                                {incident.name}
                                            </p>
                                            <p className="text-muted"><span className='font-semibold'>Created at: </span>{new Date(incident.createdAt).toLocaleString()}</p>
                                            {incident.resolvedAt && (
                                                <p className="text-muted"><span className='font-semibold'>Resolved at: </span>{new Date(incident.resolvedAt).toLocaleString()}</p>
                                            )}
                                            {incident.history.length > 0 && (
                                                <div className="mt-2">
                                                    <h4 className="font-semibold text-muted text-lg">Incident History</h4>
                                                    {incident.history.sort((a, b) => b.id - a.id).map((status) => (
                                                        <p key={status.id} className="text-muted flex flex-col">
                                                            <span>
                                                                <span className='font-semibold text-secondary'>{status.status === 0 ? "Investigating" : status.status === 1 ? "Update" : status.status === 2 ? "Identified" : status.status === 3 ? "Monitoring" : status.status === 4 ? "Resolved" : ""}</span> - {status.statusMessage}<br />
                                                            </span>
            ...
            <footer className='flex justify-center pb-8'>
                <p className='text-gray-400 text-sm'>Powered by WatchTower</p>
            </footer>
        </>
    );
}